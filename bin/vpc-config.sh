#!/usr/bin/env bash

set -euo pipefail

beattyga_pw=$1

apt update

export DEBIAN_FRONTEND=noninteractive

apt-get -o "Dpkg::Options::=--force-confold" dist-upgrade -q -y --force-yes

apt install -y tree git tmux

#-----------------------------------------------------------------------------------------------------
# add user: beattyga
#-----------------------------------------------------------------------------------------------------

useradd -m -s /bin/bash beattyga

echo "beattyga:${beattyga_pw}" | chpasswd

# /bin/rm ~beattyga/.profile
# /bin/rm ~beattyga/.bashrc
#
# cat <<EOF > ~beattyga/.bash_profile
# source $HOME/.bashrc
# EOF
#
# cat <<EOF > ~beattyga/.bashrc
# case \$- in
#   *i*) ;;
#   *)   return ;;
# esac
#
# PS1='$ '
# EOF

#-----------------------------------------------------------------------------------------------------
# add user: countskm
#-----------------------------------------------------------------------------------------------------

useradd -m -s /bin/bash countskm

mkdir -p /home/countskm/.ssh

cat <<EOF > /home/countskm/.ssh/authorized_keys
ssh-rsa  countskm@digicat.org-2014
EOF

chown -R countskm:countskm /home/countskm/.ssh
chmod -R u=rX,go=-rwx /home/countskm/.ssh

#-----------------------------------------------------------------------------------------------------
# config sudoers
#-----------------------------------------------------------------------------------------------------

cat <<EOF > /etc/sudoers.d/local
beattyga ALL=NOPASSWD: ALL
countskm ALL=NOPASSWD: ALL
EOF

#-----------------------------------------------------------------------------------------------------

cat <<EOF > /etc/ssh/sshd_config.d/50-cloud-init.conf
PasswordAuthentication yes
EOF

# this does not appear to be needed with ^^^
# service ssh restart
